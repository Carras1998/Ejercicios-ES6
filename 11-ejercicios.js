...
for (const user of users) {
  for (const soundName in user.favoritesSounds) {
    if (favoriteCount[soundName]) {
      favoriteCount[soundName]++
    } else {
      favoriteCount[soundName] = 1
    }
  }
}

for (const soundName in favoriteCount) {
  console.log(
    `El sonido ${soundName} es favorito ${favoriteCount[soundName]} veces`
  )
}
